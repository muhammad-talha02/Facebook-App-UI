import React from 'react'
import "./register.css"
const Register = () => {
    return (
        <div className="container-fluid register vh-100 d-flex align-items-center">

            <div className='container-lg'>
                <div className="row d-flex justify-content-center">
                    <div className="col-4 d-flex justify-content-center flex-column">
                        <h1 className='text-theme fw-bold display-4'>SharVibes</h1>
                        <p className='fw-bold h5 pe-3'>Make and Share Vibes around the world with ShareVibes.</p>
                    </div>
                    <div className="col-4 bg-white rounded p-4">
                        <div>
                            <div className="mb-3">
                                <input type="email" className="form-control p-2" id="exampleInputEmail1" placeholder='Username' />
                            </div>
                            <div className="mb-3">
                                <input type="email" className="form-control p-2" id="exampleInputEmail1" placeholder='Email' />
                            </div>
                            <div className="mb-3">
                                <input type="password" className="form-control p-2" id="exampleInputPassword1"  placeholder='Password'/>
                            </div>
                            <div className="mb-3">
                                <input type="password" className="form-control p-2" id="exampleInputPassword1" placeholder='Confirm Password' />
                            </div>
                            <button type="submit" className="btn btn-theme w-100">Signup</button>
                            <p className='text-center text-theme my-4'>Already have Account?</p>
                            <button type="submit" className="btn btn-success w-100">Login to Account</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Register